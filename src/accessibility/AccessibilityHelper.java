...
public class AccessibilityHelper {
    ...
    /** 查找兄弟节点*/
    @TargetApi(Build.VERSION_CODES.JELLY_BEAN_MR2)
	public  static AccessibilityNodeInfo findNodeInfosByFriendNext(AccessibilityNodeInfo nodeInfo) {
    	if(nodeInfo==null)return null;
    	AccessibilityNodeInfo parent=nodeInfo.getParent();
    	if(parent==null)return null;
    	int count=parent.getChildCount();
    	if(count<2)return null;
    	if(nodeInfo.equals(parent.getChild(parent.getChildCount()-1)))return null;
    	//if(index<=0)return parent.getChild(count-1);
    	//if(index>=count)return parent.getChild(count-1);
